import { getSyncDashboardSummary, type SyncDeliveryStatus } from "@/lib/sync/repository";
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
          <header className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-zinc-400">
                  Podglad zapisanych snapshotow i ostatnio widzianych urzadzen.
                </p>
              </div>
            ...
            <p className="mt-4 text-xs text-zinc-500">
              Ostatnia aktualizacja widoku: {formatDate(summary.generatedAt)}
            </p>
          </header>

          <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
              <p className="text-xs text-zinc-400">Uzytkownicy</p>
              <p className="mt-1 text-2xl font-semibold">{summary.userCount}</p>
            </div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
              <p className="text-xs text-zinc-400">Snapshoty (przeslane)</p>
              <p className="mt-1 text-2xl font-semibold">{summary.snapshotCount}</p>
            </div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
              <p className="text-xs text-zinc-400">Urzadzenia</p>
              <p className="mt-1 text-2xl font-semibold">{summary.deviceCount}</p>
            </div>
          ...
          <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-400">
              ...
              zgloszony stan urzadzenia. "Odebrane / aktualne" jest wyliczane na
              podstawie ostatnio wyslanego `clientRevision`/`clientHash`.
            </p>
          </section>
          ...
          <section className="space-y-4">
            {summary.users.length === 0 ? (
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-8 text-center text-zinc-400">
                Brak danych sync. Po pierwszym `push` snapshot pojawi sie tutaj.
              </div>
            ) : (
          ...
          <footer className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 text-xs text-zinc-500">
            <div>Store: {summary.storeFile}</div>
            <div>Data dir: {summary.dataDir}</div>
            <div>Endpointy API: /api/health, /api/sync/status, /api/sync/push, /api/sync/pull</div>
          </footer>
        </div>
      </main>
    );
